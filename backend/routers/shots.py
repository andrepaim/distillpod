...
from models import Shot, ShotRequest
from services.snip_engine import create_shot

router = APIRouter(prefix="/shots", tags=["shots"])


@router.post("/")
async def make_shot(req: ShotRequest, summary: bool = False) -> Shot:
    """
    Create a shot at the current playback position.
...
@router.get("/")
async def list_shots(episode_id: str = None) -> list[Shot]:
    db = await get_db()
    if episode_id:
        rows = await db.execute_fetchall(
            "SELECT * FROM shots WHERE episode_id = ? ORDER BY created_at DESC", (episode_id,)
        )
    else:
        rows = await db.execute_fetchall("SELECT * FROM shots ORDER BY created_at DESC")
    await db.close()
